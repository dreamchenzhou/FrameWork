 ...
 * 
 */
public class XCTransRegions implements ICoordTrans
{
	...
	private static void M_DD2GS(double B, double L, TranItem item)
	{
		...
		l = L - (114 / 180.0) * Math.PI;
		t = Math.tan(B);
		Xita = E1 * Math.cos(B);

		item.x80 = X0 + (N / 2) * Math.sin(B) * Math.cos(B) * l * l + (N / 24) * Math.sin(B) * Math.pow(Math.cos(B), 3) * (5 - t * t + 9 * Xita * Xita + 4 * Math.pow(Xita, 4)) * Math.pow(l, 4) + (N / 720) * Math.sin(B) * Math.pow(Math.cos(B), 5) * (61 - 58 * t * t + Math.pow(t, 4)) * Math.pow(l, 6);
	...
	public static void BL2xy(double B, double L, TranItem item)
	{
		...
		item.x80 = tran54.x80 + 21.16;
		item.y80 = tran54.y80 + 79.81;
	}
}